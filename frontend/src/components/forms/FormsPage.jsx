  ...
  return (
    <div style={{ minHeight:'100vh', background:BRAND.paper, fontFamily:'inherit' }}>
      ...
      <div style={{ background:BRAND.heroBg, padding: isMobile ? '14px 16px 12px' : '28px 32px 24px', color:'#fff' }}>
        ...
        <h1 style={{ fontSize: 28, fontWeight:800, margin:'0 0 4px', letterSpacing:'-0.02em' }}>表單管理</h1>
        <p style={{ fontSize: 15, opacity:0.7, margin:0 }}>建立資料蒐集表單，將外部需求直接轉換為任務或記錄</p>
      ...
      <div style={{ padding: isMobile ? '14px 16px' : '24px 32px' }}>
        ...
        {!loading && (
          <div style={{ background:BRAND.white, borderRadius:10, border:`1px solid ${BRAND.mist}`, overflowX:'auto' }}>
            ...
            {forms.length === 0 && (
              <div style={{ padding:'40px 16px', textAlign:'center', color:BRAND.muted, fontSize: 15 }}>尚無表單</div>
            )}
              ...
              <div key={f.id}>
                ...
                >
                  ...
                  <span style={{ display:'inline-flex', alignItems:'center', gap:5 }}>
                    ...
                    <span style={{ fontSize: 14, color: f.status === 'active' ? '#16824B' : BRAND.muted }}>
                      {f.status === 'active' ? '啟用' : '停用'}
                    </span>
                  </span>
                  <span style={{ fontSize: 15, color:BRAND.ink, textAlign:'right' }}>{f.submissions || 0}</span>
                  <span style={{ fontSize: 14, color:BRAND.muted }}>{formatDate(f.lastSubmit)}</span>
                ...
                {expanded === f.id && (
                  <div style={{ padding:'14px 20px 18px', background:BRAND.surfaceMuted, borderBottom:`1px solid ${BRAND.mist}`, borderLeft:`3px solid ${BRAND.silver}` }}>
                    <div style={{ fontSize: 14, color:BRAND.carbon, marginBottom:10 }}>{f.description || '（無說明）'}</div>
                    <div style={{ fontSize: 13, fontWeight:600, color:BRAND.muted, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:8 }}>最近提交</div>
      ...
      {showModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
          <div style={{ background:BRAND.white, borderRadius:12, padding: isMobile ? '14px 16px' : '28px 32px', width:480, maxWidth:'90vw' }}>
            <h2 style={{ fontSize: 20, fontWeight:700, margin:'0 0 20px', color:BRAND.ink }}>建立表單</h2>
