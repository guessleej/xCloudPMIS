...
xCloudPMIS 採三層角色制度：**管理員（admin）> 專案經理（pm）> 成員（member）**。

### 權限矩陣

| 功能                         | admin | pm（僅自己的） | member |
|------------------------------|:-----:|:------------:|:------:|
| 建立專案                      | ✓     | ✓            | ✓      |
| 編輯專案                      | ✓     | 僅自己建立的  | 僅自己建立的 |
| 刪除專案                      | ✓     | 僅自己建立的  | 僅自己建立的 |
...
> **PM 備註：** 「僅自己的」= 該專案的 `ownerId` 或 `createdById` 等於當前用戶 ID。
...
| 步驟 | 說明 |
|------|------|
| Email 登入 | 使用公司 Email + 密碼，首次登入建議立即至「設定 → 個人設定」修改密碼 |
| OAuth 登入 | 支援 Microsoft / Google / GitHub 一鍵登入，需管理員預先設定 OAuth App |
| 個人設定 | 修改顯示名稱、頭像、通知偏好、時區 |
| 主題切換 | 右上角太陽/月亮圖示，支援深色 / 淺色模式 |
...
- **KPI 卡片**：總專案數、進行中任務、逾期任務、本月完成數
- **專案健康度**：甜甜圈圖，按計劃 / 有風險 / 逾期佔比
- **AI 洞察提醒**：自動偵測風險並給出建議
- **我的任務**：當前使用者指派任務快速預覽
...
左側導覽 → **所有專案**，顯示公司全部（未刪除）專案。

| 操作 | 說明 | 角色限制 |
|------|------|----------|
| 建立專案 | 點擊「+ 新增專案」，填寫名稱、狀態、負責人、日期等 | 全部 |
| 編輯專案 | 點擊列表最右側「✏ 編輯」按鈕 | admin / 自己的（pm/member） |
| 刪除專案 | 點擊「🗑 刪除」按鈕，軟刪除（可從資源回收筒復原） | admin / 自己的（pm/member） |
| 永久刪除 | 資源回收筒 → 永久刪除 | admin 限定 |

#### 4.2 我的專案

左側導覽 → **我的專案**，篩選顯示「由我建立」的專案。支援快速篩選：

- **我建立的**：`createdById` = 當前用戶
- **被指派的**：用戶是成員但非建立者的專案
...
| 視圖 | 說明 |
|------|------|
| **看板（Kanban）** | 依任務狀態（待辦 / 進行中 / 審核中 / 已完成）的拖拉式看板 |
| **列表（List）** | 任務表格，支援排序、一次看到所有任務屬性 |
| **里程碑（Milestones）** | 時間軸呈現里程碑，可標記達成 |

#### 4.4 專案成員管理（Member Management）

在專案詳情頁的**頂部標題列右側**，會顯示所有成員的頭像 + 姓名標籤列：

| 操作 | 說明 | 角色限制 |
|------|------|----------|
| 查看目前成員 | 標題列右側列出所有成員 Avatar pill | 全部 |
| ✕ 移除成員 | 點擊成員 pill 右側的 `×` 直接移除 | admin / 自己建立的專案（pm） |
| ＋ 加入成員 | 點擊「＋ 加入成員」展開下拉選單，勾選後即時加入 | admin / 自己建立的專案（pm） |

> 成員列表變更（加入 / 移除）會立即呼叫後端 `PATCH /api/projects/:id` 並重新載入最新資料，頁面顯示 Toast 提示結果。

---

### 5. 任務管理

點擊任意任務卡片（或列表中的「⋯」按鈕）開啟任務側邊欄，支援：

| 欄位 / 操作 | 說明 | 角色限制 |
|-------------|------|----------|
| 標題內嵌編輯 | 直接點擊修改 | admin / pm |
| 指派人員 | 多人指派，選取成員 | admin / pm |
| 狀態、優先級、日期 | 下拉/日期選取器 | admin / pm |
| 子任務 | 快速新增子任務樹 | admin / pm（成員可完成） |
| 待辦清單 | 加入 / 勾選 / 刪除 Checklist | 新增/刪除：admin/pm；勾選：全部 |
| 附件 | 拖拉或點擊上傳 | admin / pm |
| 留言 | 富文字留言，支援 @提及 | 全部 |
| 自訂欄位 | 動態文字 / 下拉 / 數字 | admin / pm |
| 刪除任務 | 關閉側邊欄並軟刪除 | admin / pm |
...
> **PM 注意：** 報表頁的「所屬專案」篩選僅列出**本人負責的專案**，不顯示其他人的專案資料。

| 報表類型 | 說明 |
|----------|------|
| 儀表板總覽 | 全公司 KPI 概覽（admin 全部；pm 僅自己的） |
| 任務報表 | 依專案 / 狀態 / 日期範圍篩選任務清單 |
| 專案報表 | 專案健康度、進度、成員負載 |
| 工時報表 | 成員工時統計，可匯出 CSV |
| 高階摘要 | 跨專案彙總，適合管理層審閱 |
...
左側導覽 → **自動化規則**（admin / pm 可存取）。

規則由「觸發條件」＋「執行動作」組成：

| 觸發類型 | 範例 |
|----------|------|
| 任務狀態變更 | 任務進入「審核中」時，自動通知指派人 |
| 截止日到達 | 距截止日 3 天時，寄送提醒通知 |
| 任務建立 | 新任務建立時，自動指派給預設負責人 |

---

### 10. 專案組合（Portfolio）

左側導覽 → **專案集**（admin / pm 可存取）。

- 將多個相關專案組成一個「組合」
- 統計卡片：總專案數 / 按計劃 / 有風險 / 逾期
- 支援每個專案的健康度循環切換、行內備註、週報填寫

---

### 11. OKR 目標（Goals）

左側導覽 → **目標**（admin / pm 可存取）。

- 建立公司級 / 部門級 / 個人級 OKR 目標
- 連結任務作為 Key Results 的量化依據
- 自動計算達成率
...
任務側邊欄或專案頁可觸發 AI 功能，所有 AI 建議均需 PM 核准後執行（Human-in-the-Loop）：

| AI 功能 | 說明 |
|---------|------|
| 任務拆解 | 輸入專案目標，AI 生成子任務清單與估時 |
| 風險分析 | 分析專案健康度並輸出風險評分（0–100）及改善建議 |
| 週報生成 | 依當週任務狀態自動撰寫 Markdown 週報 |
| 排程優化 | 提供關鍵路徑分析和資源重新分配建議 |

> AI 決策狀態流程：`pending → 審核中 → approved/rejected → executing → completed`
...
- 自動化規則觸發結果
- AI 決策待審核提醒
- 未讀數量徽章即時更新（Socket.IO）

---

### 14. 團隊管理

左側導覽 → **我的任務** 旁的人像圖示（admin / pm 可存取）。

| 操作 | admin | pm |
|------|:-----:|:--:|
| 查看成員列表 | ✓ | ✓ |
| 編輯成員資訊（姓名/職稱/部門） | ✓ | ✓ |
| 變更角色（admin/pm/member） | ✓ | ✗ |
| 停用 / 啟用帳號 | ✓ | ✗ |
...
左側導覽最下方齒輪圖示（admin 限定）：

- **公司設定**：名稱、Logo、時區
- **AI 模型設定**：選擇 OpenAI 模型、設定 API Key 與 baseURL
- **通知設定**：設定郵件、Webhook 等通知渠道
