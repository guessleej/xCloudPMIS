...
router.get('/task/:taskId/ics', async (req, res) => {
  ...
  try {
    const task = await prisma.task.findUnique({
      where: { id: taskId },
      select: {
        id: true, title: true, description: true, dueDate: true, dueEndDate: true, dueTime: true, dueEndTime: true,
    ...
    const dueEndDateVal = task.dueEndDate ? new Date(task.dueEndDate) : dueDate;
    ...
    const endDate   = buildTzDate(dueEndDateVal, endHour, endMinute || 0);
...
router.get('/task/:taskId/add', async (req, res) => {
  ...
  try {
    ...
    const endDateTime   = buildTzDate(dueEndDateVal, endHour, endMinute || 0);
